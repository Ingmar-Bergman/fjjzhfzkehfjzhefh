...
#             # data = data.to(device)
#             # torch.nn.utils.clip_grad_norm_(autoencoder.parameters(), 0.1)
#             # loss, recon, kld  = autoencoder.loss_function(data)
#             # loss.backward()
...
denoise_model = DenoiseNN(input_dim=args.latent_dim, hidden_dim=args.hidden_dim_denoise, n_layers=args.n_layers_denoise, n_cond=args.n_condition, d_cond=args.dim_condition).to(device)
base_optimizer = torch.optim.Adam
optimizer = SAM(denoise_model.parameters(), base_optimizer, adaptive=True, lr = 0.001,rho=0.075) #0.019 pour rho = 0.1
...
if args.train_denoiser:
    ...
    # **Load the best denoiser after training**
